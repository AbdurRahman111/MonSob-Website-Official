...
def checkout_func(request):
    return render(request, 'boomboom_user/checkout_new.html')
...
def order_save_with_all_info(request):
    ...
    if request.user.is_authenticated:
        authinticated_user = request.user
    else:
        authinticated_user = None

    # try:
    var_customer_info = Customer_delivery_information(Customer=authinticated_user, First_Name=customer_first_name, Last_Name=customer_last_name, Street_Address=customer_Street_Address, Town_City=customer_Town_City, District=customer_District, Post_Code=customer_Post_Code, Phone_Number=customer_Phone_Number, Email_Address=customer_Email_Address)
    var_customer_info.save()
    # except Exception as e:
    #     var_customer_info = None
